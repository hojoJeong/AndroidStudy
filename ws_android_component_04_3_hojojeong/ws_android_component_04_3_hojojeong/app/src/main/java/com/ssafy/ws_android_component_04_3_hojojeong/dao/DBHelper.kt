package com.ssafy.ws_android_component_04_3_hojojeong.dao
...
class DBHelper(
    ...
    fun selectAll(): MutableList<StuffDto> {
        ...
        db.rawQuery("select * from $TABLE", null).use {
            while (it.moveToNext()) {
                ...
                stuffList.add(StuffDto(id, stuffItem, quantity))
            }
        }
        return stuffList
    }
    ...
    fun selectById(getId: Int): StuffDto? {
        ...
        db.rawQuery("select * from $TABLE where _id = ?", arrayOf(getId.toString())).use {
            while (it.moveToNext()) {
                ...
                stuffData = StuffDto(id, stuffItem, quantity)
            }
        }
        return stuffData
    }
    ...
    companion object {
        ...
        private const val TABLE_CREATE = "CREATE TABLE if not exists $TABLE " +
                "(_id integer primary key autoincrement, stuffName text, quantity integer);"
    }
}

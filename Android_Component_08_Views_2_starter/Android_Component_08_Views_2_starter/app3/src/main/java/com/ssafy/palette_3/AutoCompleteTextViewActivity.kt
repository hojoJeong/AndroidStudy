...
import com.ssafy.palette_3.databinding.ActivityAutoCompleteBinding

private const val TAG = "AutoCompleteTextViewAct 싸피"
...
//            mainAutoCompleteTv.setAdapter(adapter)
            ...
            //검색창에 입력할 때마다 서버에서 목록 가져올 경우
            mainAutoCompleteTv.addTextChangedListener {
                val filtered = COUNTRIES.filter { country ->
                    country.startsWith(it.toString())
                }
                Log.d(TAG, "onCreate: ${filtered}")
                mainAutoCompleteTv.setAdapter(ArrayAdapter(this@AutoCompleteTextViewActivity,android.R.layout.simple_dropdown_item_1line, filtered))
            }
            mainAutoCompleteTv.setOnItemClickListener { parent, view, position, id ->
